...
export const usesStdinStdout = (mode: string): boolean => mode === "claude";

export const getCliStrategy = (mode: CLIType): CliStrategy =>
  CLI_STRATEGIES[mode];
...
export function buildCliInvocation(
  ...
  if (!strategy.baseCommand || strategy.baseCommand.length === 0) {
    throw new Error(
      `CLI strategy "${strategy.id}" does not define a base command`
    );
  }
